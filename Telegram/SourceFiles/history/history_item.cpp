...
: HistoryItem(history, {
	...
	if (checked == MediaCheckResult::Unsupported) {
		...
		if (const auto richMessage = data.vrich_message()) {
			const auto richPage = Iv::ParseRichPage(&history->session(), *richMessage);
			setRichPage(richPage);
			_originalMsg = Iv::FlattenRichPageSummary(richPage);
			_blockMsg = applyBlockedUserSpoiler(_originalMsg);
			const auto textWithEntities = isBlocked ? _blockMsg : _originalMsg;
